...
const BlogList = ({ data, isLoading }: BlogListProps) => {
  ...
  const techBlogs = data?.filter(
    (blog) => blog.category?.toLowerCase() === "tech"
  );
  const astronomyBlogs = data?.filter(
    (blog) => blog.category?.toLowerCase() === "astronomy"
  );
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      {tabValue === 0 && (
        <Grid container spacing={3}>
          {techBlogs.length > 0 ? (
            techBlogs.map((blog: IBlog, index: number) => (
              <Grid size={{ xs: 12, sm: 6, md: 4 }} key={blog.id || index}>
                <BlogCard
                  id={blog.id}
                  imageUrl={blog.imageUrl || ""}
      ...
      {tabValue === 1 && (
        <Grid container spacing={3}>
          {astronomyBlogs.length > 0 ? (
            astronomyBlogs.map((blog: IBlog, index: number) => (
              <Grid size={{ xs: 12, sm: 6, md: 4 }} key={blog.id || index}>
                <BlogCard
                  id={blog.id}
                  imageUrl={blog.imageUrl || ""}
